...
function createSidebar() {
  ...
  header.innerHTML = `
    <div class="sidebar-header-left">
      <img src="${chrome.runtime.getURL('icons/logo_48.png')}" alt="Logo" class="sidebar-logo">
      <span class="sidebar-title">${chrome.i18n.getMessage('extensionName')}</span>
    </div>
    <button class="sidebar-close-btn" title="关闭">
  ...
  console.log('Chat-Memo: 注入式侧边栏已创建');
  return sidebar;
}
...
async function toggleSidebar(force) {
  ...
  if (shouldOpen) {
    ...
    if (sidebar) {
      sidebar.remove();
      console.log('Chat-Memo: 侧边栏已从DOM移除');
    }
  }
}
